...
/**
 * Install global error/promise-rejection guards that swallow the spurious
 * "Unable to activate keep awake" errors raised by expo-keep-awake during
 * passkey/system-dialog transitions.
 */
export function globalPolyfill() {
  ...
  // Hermes-specific: swallow the same error when it surfaces as a promise rejection.
  // @ts-ignore
...
/**
 * Polyfill `global.navigator.credentials` + WebAuthn response globals on top
 * of `react-native-passkey` so Liquid Auth code can call DOM-style WebAuthn.
 */
